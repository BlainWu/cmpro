...
Model::Model() :config(){
    input_size = config.TENSOR_SHAPE;
    output_size = config.TENSOR_OUTPUT;
...
int Model::model_predict(std::vector<double> dataori) {
    ...
    int ans=0;
    tensorflow::Tensor tinput(tensorflow::DT_FLOAT, tensorflow::TensorShape({1, input_size}));
    assign("input", &dataori);

    std::vector<tensorflow::Tensor> outputs;
    auto dst = tinput.flat<float>().data();
    copy_n(dataori.begin(), input_size, dst);
